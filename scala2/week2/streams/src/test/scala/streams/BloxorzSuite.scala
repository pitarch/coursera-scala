...
class BloxorzSuite {
  ...
  @Test def `isStanding should return true when standing`: Unit = {
    new Level1 {
  ...
  @Test def `isStanding should return false when not standing`: Unit = {
    new Level1 {
  ...
  @Test def `test isLegal`: Unit = {
    new Level1 {
      ...
      assert(Block(Pos(0, 2), Pos(0,2)).isLegal)
