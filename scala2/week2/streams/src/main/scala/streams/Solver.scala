...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      case LazyList.cons((block, history), tl)  => {
        val neighbors = block.legalNeighbors.map( _._1).toSet
        val nextNeighborsWithHistory = newNeighborsOnly(neighborsWithHistory(block, history), explored)
        (block, history) #:: from(tl #::: nextNeighborsWithHistory,  explored ++ neighbors )
      }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.take(1).toList match {
    case Nil => Nil
