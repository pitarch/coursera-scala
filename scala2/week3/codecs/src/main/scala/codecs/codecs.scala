...
trait DecoderInstances {
  ...
  Decoder.fromPartialFunction {
    case Json.Num(bd) if bd.isValidInt => bd.toInt
  }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) =>
        val options = items.map(decoder.decode)
        if (options.exists(_.isEmpty)) None else Some(options.map(_.get))
      case _ => None
    }
...
trait ContactsCodecs {
   ...
   */
  implicit val contactsDecoder: Decoder[Contacts] =  Decoder.field[List[Person]]("people").transform[Contacts] {
    case people@_ => Contacts(people)
  }
}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val p1 = Person(name = "foo", age = 12)
    val json1 = Json.Obj(fields = Map("name" -> Json.Str("Alice"), "age" -> Json.Num(42)))
    val encodedP1 = implicitly[Encoder[Person]].encode(p1)
    val decodedJson1 = implicitly[Decoder[Person]].decode(json1)
    val x1 = implicitly[Encoder[Contacts]].encode(Contacts(List(Person(name = "Alice", age = 41))))
    val x2 = implicitly[Encoder[Contacts]].encode(Contacts(List.empty))
    val x2x = implicitly[Decoder[Contacts]].decode(x2)
    println(encodedP1)
    //    println(s"--> ${p1}")
    //     println(maybeJsonObj.flatMap(_.decodeAs[Person]))
    // should be none because age is not an Int
