...
import scala.collection.parallel.{ParMap, ParSeq, immutable}
...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    val closestMap = points.groupBy(p => findClosest(p, means))
    means.map { mean => (mean, closestMap.getOrElse(mean, Seq[Point]())) }.toMap
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    val closestMap: immutable.ParMap[Point, ParSeq[Point]] = points.groupBy(p => findClosest(p, means))
    means.map { mean => (mean, closestMap.getOrElse(mean, ParSeq[Point]())) }.toMap
  }
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] = {
    //    oldMeans.map { mean => findAverage(mean, classified.getOrElse(mean, ParSeq())) }
    oldMeans.map { mean => findAverage(mean, classified(mean)) }
  }

  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    val result = oldMeans.zip(newMeans)
      .map { case (oldMean, newMean) => oldMean.squareDistance(newMean) }
      .sum
    //    println(s"converged result: $result (<= $eta?)")
    Math.sqrt(result) <= eta
  }
  ...
  final def kMeans(points: Seq[Point], means: Seq[Point], eta: Double): Seq[Point] = {
    ...
    val done = converged(eta, means, newMeans)
    //    println(s"classification: $classification")
    //    println(s"newMeans: $newMeans")
    //    println(s"converged: $done")
    if (!done) kMeans(points, newMeans, eta) else newMeans // your implementation need to be tail recursive
  }
