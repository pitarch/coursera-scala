...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val sc: SparkContext = new SparkContext("local[*]", "TotalSpentByCustomerSorted", conf = conf)
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def seqOp: (Int, WikipediaArticle) => Int = (acc: Int, article: WikipediaArticle) => if (article.mentionsLanguage(lang)) acc + 1 else acc
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val result = rdd.flatMap(a => langs.map(lang => (lang, a)))
