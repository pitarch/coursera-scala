...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeeds = columnNames.filter(name => Seq("t01", "t03", "t11", "t1801", "t1803").exists(name.startsWith))
    val work = columnNames.filter(name => Seq("t05", "t1805").exists(name.startsWith))
    val other = columnNames.filter(name => !work.contains(name) && !primaryNeeds.contains(name) && Seq("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(name.startsWith))
    (primaryNeeds map col, work map col, other map col)
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    val avgDF: Dataset[TimeUsageRow] = summed
